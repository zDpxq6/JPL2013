...
public class MakingArrayScreen extends JFrame {
	...
	private void init(MakingArrayScreen 配列入力画面){
		...
		配列入力画面.setSize(Const.コンストラクタパラメータ入力UI幅, Const.コンストラクタパラメータ入力UI高さ);
	...
	private void addPanels(int パネル数){
		...
		for (JPanel element : result) {
			this.配列要素入力コンボボックスアレイ[i] = コンボボックス生成(this.mdls.get生成済オブジェクトのコンボボックスモデル());
			JPanel tmp = new CommonInputPanel(new JLabel("要素" + i + ": "), this.配列要素入力コンボボックスアレイ[i], Const.OPENING_SCREEN_MAKE_OBJECT_BUTTON_LABEL, null);//定数.リフレクションスタートUIオブジェクト生成開始ボタンラベル, new リフレクションスタートUIオブジェクト生成開始ボタンリスナ()));
			element = tmp;
			this.add(element);
			i++;
		}
